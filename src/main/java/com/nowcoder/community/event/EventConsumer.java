...
public class EventConsumer implements CommunityConstant {
    ...
    public void handleShareMessage(ConsumerRecord record) {
        ...
        //启用定时器，监视该图片，一旦生成，则上传到qiniuCloud
    ...
    class UploadTask implements Runnable {
        ...
        public void run() {
            ...
            if (System.currentTimeMillis() - startTime > 300000) {
                log.error("执行任务时间过长,终止任务：" + fileName);
                future.cancel(true);
                return;
            }
            //上传失败
            if (uploadTimes > 5) {
                log.error("上次任务次数过多，终止任务：" + fileName);
                future.cancel(true);
                return;
            }
            ...
            if (file.exists()) {
                ...
                try {
                    //开始上传图片
                    Response response = manager.put(path, fileName, uploadToken, null, "png", false);
                    //处理响应结果
                    JSONObject json = JSONObject.parseObject(response.bodyString());//将结果转成json字符串
                    if (json == null || json.get("code") == null || !json.get("code").toString().equals("0")) {
                        log.info(String.format("第%d次上传失败[%s].", uploadTimes, fileName));
                    } else {
                        log.info(String.format("第%d次上传成功[%s].", uploadTimes, fileName));
                        future.cancel(true);//关闭定时任务
                    }
